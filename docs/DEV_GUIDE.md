...
> 本文档基于 [PRD.md](./PRD.md) 编写，采用**单用户服务端部署方案**：前端负责交互与展示，后端负责 AI 请求代理、数据持久化与配置管理，使用 **OpenAI 兼容格式 API + SQLite 数据库**。
...
│   │   ├── Toolbar.tsx            # 顶部工具栏
│   │   ├── StatusBar.tsx          # 底部状态栏
│   │   ├── SettingsModal.tsx      # 设置弹窗（模型、服务地址等）
...
**目标**：定义所有核心类型，创建前端 Zustand Store 与服务端 SQLite 表结构，预置五行基础元素，并打通首屏加载与写库流程。
...
- [x] **元素库 Store** `src/store/elementStore.ts`
  ...
  - 操作：`hydrate()`、`addElement()`、`getElement()`、`getAllElements()`、`searchElements(query)`
  - 初始化时预置五行基础元素
  - 首屏通过 `/api/state` 拉取数据库内容并写入前端内存态
...
> 建议将 `categories`、工作台布局等复杂字段以 JSON 字符串形式存储；前端只保留运行时缓存，权威数据以数据库为准。

### 验收标准

- 控制台可读取到 5 个基础元素
- 刷新页面或重启前端后数据仍然存在（SQLite 持久化生效）
- 添加新元素后可正常检索
...
**目标**：封装服务端 AI 调用，实现 Prompt 构造、响应解析、重试机制，以及前后端配置联动。
...
- [x] **API 调用实现**
  - 前端调用 `POST /api/craft`
  - 服务端再使用 `fetch` 调用 `POST {baseUrl}/v1/chat/completions`
  - 服务端请求头：`Authorization: Bearer ${process.env.OPENAI_API_KEY}`、`Content-Type: application/json`
...
**API Key 安全提示**：API Key 应仅存放在服务器环境变量中，由后端代理调用模型接口。前端只访问自己的服务端接口，不直接接触第三方模型密钥。
...
- [x] **完善合成流程** `src/hooks/useCraft.ts`
  ```
  合成流程：
  1. 归一化输入（排序元素名）
...
- [x] **合成动画组件** `src/components/CraftAnimation.tsx`
  - **合成中动画**（Framer Motion）：
...
- [ ] **API 相关**
  - 服务端未配置 `OPENAI_API_KEY` 时给出明确错误提示
...
- [ ] **构建与部署**
  - `pnpm build` 生成前端静态文件
  - 构建并启动 Node.js 服务端
  - 部署目标：个人 VPS / 云服务器 / Docker 单机环境
  - 配置环境变量：`OPENAI_API_KEY`、数据库路径、服务端口
...
server/index.ts                  — 服务端启动与部署入口
server/db/client.ts              — 数据库连接与路径配置
.env.example                     — 环境变量示例
```
